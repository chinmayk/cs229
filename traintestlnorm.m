...
compareaccs = zeros(size(courseids,2),10);
for i=1:1%size(courseids,2)
    ...
    [labels, features] = libsvmread(['newcourse',num2str(courseids(i))]);
    for l=1:size(labels,1)
        labels(l) = gradeMap(labels(l));
    end
    ...
    meanguess = repmat(mean(labels), size(labels));
    ...
    baseline2_guesses(isnan(baseline2_guesses)) = meanguess(1);
   ...
   for x=1:10
    ...
    crossrms = crossrms/10.0;
    crossl2 = crossl2/10.0;
    ...
    compareaccs(i,:) = [crossacc, crossprec, crossl2, crossrms, baseline_acc, baseline_l2, baseline_rms, baseline2_acc, baseline2_l2, baseline2_rms];
...
%highprobacc
dlmwrite('multisvm_comp.csv', compareaccs);
